...
# This script will enumerate all of the disabled users in the Disabled Users OU of FAKECORP.
# dsquery user "OU=DisabledAcctUsers,OU=UserObjects,OU=acme,DC=FAKECORP,DC=adcmax,DC=acme,DC=org" -limit 0 | dsget user -samid -disabled -fn -ln -display | Find /V "dsget succeeded"| Find /V "  samid"| Out-File $SaveFilePath$FileName -Append
$userList=dsquery user "OU=DisabledAcctUsers,OU=UserObjects,OU=acme,DC=FAKECORP,DC=adcmax,DC=acme,DC=org" -limit 0 | dsget user -samid -disabled -fn -ln | Find /V "dsget succeeded"| Find /V "  samid"
...
#--------------------------------------Separates the FAKECORP and DMX versions of this script----------------------------------------

# This script will enumerate all of the disabled users in the Disabled Users OU of DMX.
# dsquery user "OU=DisabledUsers,OU=CarMaxUsers,OU=acme,DC=DMX,DC=LOCAL" -limit 0 | dsget user -samid -disabled -fn -ln -display | Find /V "dsget succeeded"| Find /V "  samid"| Out-File $SaveFilePath$FileName -Append
$user2List=dsquery user "OU=DisabledUsers,OU=CarMaxUsers,DC=DMX,DC=LOCAL" -limit 0 | dsget user -samid -disabled -fn -ln | Find /V "dsget succeeded"| Find /V "  samid"
