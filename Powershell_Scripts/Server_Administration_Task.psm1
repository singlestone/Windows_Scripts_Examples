...
Function NET_INT_CONF() {
	...
	Write-Host "$((Get-Date).ToString('MMyyyyddhhmmss')) : Configuring NIC $InterfaceAlias_LF....."

	Connect-VIServer `
		-Server $NIC_vsphere `
		-Credential $mycredentials_NC
...
'@
	...
	$Script = $Script.Replace('<#IP_Address_LF#>',$IP_Address_LF)
	$Script = $Script.Replace('<#InterfaceAlias_LF#>',$InterfaceAlias_LF)
	$Script = $Script.Replace('<#Subnetmask_LF#>',$Subnetmask_LF)
	$Script = $Script.Replace('<#Gatway_LF#>',$Gatway_LF)
	$Script = $Script.Replace('<#DNS_Server_Prime_LF#>',$DNS_Server_Prime_LF)
	$Script = $Script.Replace('<#DNS_Server_Sec_LF#>',$DNS_Server_Sec_LF)	
	...
	Invoke-VMScript `
		-ScriptText $Script `
		-VM $MyServer `
		-HostCredential $mycredentials_NC `
		-GuestCredential $Hostcredentials
					 
	Write-Host "$((Get-Date).ToString('MMyyyyddhhmmss')) : NIC $InterfaceAlias_LF has been configured to use the static IP: $IP_Address_LF"
	Write-Host "$((Get-Date).ToString('MMyyyyddhhmmss')) : Configuration is a follows..."
	Write-Host "$((Get-Date).ToString('MMyyyyddhhmmss')) : Gateway: 		=		$Gatway_LF"
	Write-Host "$((Get-Date).ToString('MMyyyyddhhmmss')) : Subnet mask: 	=		$Subnetmask_LF"
	Write-Host "$((Get-Date).ToString('MMyyyyddhhmmss')) : DNS Primary: 	=		$DNS_Server_Prime_LF"
	Write-Host "$((Get-Date).ToString('MMyyyyddhhmmss')) : DNS Secondary: 	=		$DNS_Server_Sec_LF"
	Write-Host "$((Get-Date).ToString('MMyyyyddhhmmss')) : NIC Configuration Complete."	 
...
Function Config_Local_HD() {
		param(
			[object]$mycredentials_NC,
			[object]$Hostcredentials,
			[string]$MyServer,
			[string]$CLH_Drive_Letter,
			[INT]$CLH_Drive_Size
			);
	$Script = @'
		Function Test_Drive() {
			...
			ElseIf ((Test-Path $Drive_Letter_LF)) {
				Write-Host "Drive $Drive_Letter_LF already exists."
				Return $false
			}
			Return $True
		}
		...
		App_Drive_CONF 	$CLH_Drive_Letter $CLH_Drive_Size
		
'@
	$Script = $Script.Replace('<#Drive_Letter_LF#>',$Drive_Letter_LF)
	$Script = $Script.Replace('<#Drive_Size_LF#>',$Drive_Size_LF)
